...
test("reuses the picker styles for the nested dialog controls", () => {
  assert.match(source, /const primaryActionStyle[\s\S]*?background: "var\(--accent\)"/);
  assert.equal(source.match(/style=\{\{ \.\.\.panelSurfaceStyle/g)?.length, 2);
  assert.equal(source.match(/style=\{\{ \.\.\.textInputStyle/g)?.length, 2);
  assert.equal(source.match(/style=\{\{ \.\.\.primaryActionStyle/g)?.length, 4);
  assert.equal(source.match(/style=\{\{ \.\.\.secondaryActionStyle/g)?.length, 2);

  const modalSource = source.slice(source.indexOf("{createOpen && ("));
  const confirmAndSelect = modalSource.slice(0, modalSource.indexOf('t("directoryPicker.confirmAndSelect")'));
  const confirm = modalSource.slice(
    modalSource.indexOf('t("directoryPicker.confirmAndSelect")'),
    modalSource.indexOf('t("directoryPicker.confirm")'),
  );
  assert.match(confirmAndSelect, /\.\.\.primaryActionStyle/);
  assert.match(confirm, /\.\.\.primaryActionStyle/);
  assert.doesNotMatch(confirmAndSelect, /opacity: creating \|\| !directoryName\.trim\(\)/);
  assert.doesNotMatch(confirm, /opacity: creating \|\| !directoryName\.trim\(\)/);
});
